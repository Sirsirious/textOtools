...
import dev.tfduque.textO.textTools.GDBConnection;
...
public class GraphEntryPoint {
	public static void main(String[] args) {
		...
		try {
			...
			graphSession = GDBConnection.getGraphDBConnection();
			...
			for (String mainTerm : allTerms) {
				...
				if (!mappedRelations.isEmpty()) {
					StatementResult reiter;
					for (Map.Entry<String, Integer> entry : sortByValue(mappedRelations).entrySet()) {
						
						Transaction tx = graphSession.beginTransaction();
						String queryString = "MERGE (node1:Concept {name: $name})"
								+ "MERGE (node2:Concept {name: $secondConcept})"
								+ "MERGE (node1)-[:RelatesTo{averageLenght: $length}]-(node2)";
						Map<String, Object> parameters = new HashMap<>();
						parameters.put("name", mainTerm);
						parameters.put("length", entry.getValue());
						parameters.put("secondConcept", entry.getKey());
						reiter = tx.run(queryString, parameters);
						tx.success();
						tx.close();
